...
Android 构建系统从最初的 GNU Make 演进到 Soong/Blueprint。理解构建系统对安全研究至关重要：它决定了模块如何编译、系统镜像如何组装、SELinux 策略如何生成。
...
> Google 曾计划用 Bazel 替代 Soong，但已于 2024 年放弃该迁移计划。当前 AOSP 仍以 Soong 为主构建系统。

老项目可能仍有 `Android.mk`，新代码推荐 `Android.bp`。
...
标准两段式（Android 13 及之前）：

```
lunch <product_name>-<build_variant>
```

示例：`lunch aosp_arm-userdebug`

Android 14+ 实验性地引入了三段式格式：
...
- **release**：发布类型，如 `trunk_staging`, `ap2a`（仅三段式）
- **variant**（亦即 **build_variant**）：构建变体
